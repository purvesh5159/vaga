	...
	if ($result > 0) {
		...
		if ($action == 'edit' && ($object->status < 3 || $object->status == 99)) {
			...
			if (!empty($object->lines)) {
				...
				print '<td class="linecolvat">GST Value</td>';  //vsn 
				...
				foreach ($object->lines as &$line) {
					...
					if ($action != 'editline' || $line->id != GETPOST('rowid', 'int')) {
						...
						print '</td>';
						$value_gst = price($line->value_unit) - price($pricenettoshow);
						print '<td class="right linecolunitttc">' .$value_gst .'</td>';  //vsn
					...
					if ($action == 'editline' && $line->id == GETPOST('rowid', 'int')) {
						...
						//$defaultVat = '13% (Tx_13_0)';  //VSN 2024-06-11
						...
						print '<td class="right">';
						print $form->load_tva('vatrate', (GETPOSTISSET("vatrate") ? GETPOST("vatrate") : $selectedvat), $mysoc, '', 0, 0, '', false, 1, $defaultVat); // Add $defaultVat as the last argument
						print '</td>';
						...
						// GST TAX
						$value_gst = $value_unit - $value_unit_ht;
						print '<td class="right inputtax">';
						print '<input type="text" min="0" class="right maxwidth50" id="value_gst" name="value_gst" value="'.dol_escape_htmltag((!empty($value_gst) ? $value_gst : 0)).'">';
						print '</td>';
			...
			if (($object->status == ExpenseReport::STATUS_DRAFT || $object->status == ExpenseReport::STATUS_REFUSED) && $action != 'editline' && $user->hasRight('expensereport', 'creer')) {
				...
				print '<td class="right expensereportcreatepriceuth">GST VALUE</td>';  //VSN
				...
				$defaultvat="13 (Tx_13_0)"; //vsn working
				...
				// GST TAX  // VSN
				print '<td border="1" class="right inputtax">';
				$value_gst = $value_unit - $value_unit_ht;
				print '<input type="text" class="right maxwidth50" id="value_gst" name="value_gst" value="'.dol_escape_htmltag((!empty($value_gst) ? $value_gst : 0)).'">';
				print '</td>';
