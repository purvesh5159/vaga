			...
			if (!empty($origin) && !empty($originid)) {
				...
				if (!$error) {
					...
					if ($object_id > 0) {
                        ...
                        // purvesh added fk_propal_origin field in proposal
						 // Update fk_propal_origin field if created from proposal
						//  if ($origin == 'propal' && $originid > 0) {
						// 	$sql_update = "UPDATE ".MAIN_DB_PREFIX."commande";
						// 	$sql_update.= " SET fk_propal_origin = ".(int)$originid;
						// 	$sql_update.= " WHERE rowid = ".(int)$object->id;
							
						// 	$resql_update = $db->query($sql_update);
						// 	if (!$resql_update) {
						// 		dol_syslog("Error updating fk_propal_origin: ".$db->lasterror(), LOG_ERR);
						// 	} else {
						// 		dol_syslog("Updated fk_propal_origin for order ".$object->ref." with proposal ID ".$originid, LOG_DEBUG);
						// 	}
						// }
		...
		*/
		...
		if ($action != 'presend' && $action != 'editline') {
			...
			if (empty($reshook)) {
				...
				// Create a purchase order purvesh
				$arrayforbutaction[] = array('lang'=>'orders', 'enabled'=>(isModEnabled("supplier_order") && $object->statut > Commande::STATUS_DRAFT && count($object->lines)), 'perm'=>$usercancreatepurchaseorder, 'label'=>'AddPurchaseOrder', 'url'=>'/fourn/commande/card.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id);
				if (isModEnabled("supplier_order") && $object->statut > Commande::STATUS_DRAFT && count($object->lines)) {
