...
/**
 *   	\file       purchaseordercustomreferencenumber_list.php
 *		\ingroup    purchaseordercustomreferencenumber
 *		\brief      List page for purchaseordercustomreferencenumber
 */
...
require_once __DIR__.'/class/purchaseordercustomreferencenumber.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('purchaseordercustomreferencenumber', 'purchaseordercustomreferencenumber', 'write');
	$permissiontodelete = $user->hasRight('purchaseordercustomreferencenumber', 'purchaseordercustomreferencenumber', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("purchaseordercustomreferencenumber")) {
	accessforbidden('Module purchaseordercustomreferencenumber not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'PurchaseOrderCustomReferenceNumber';
	$objectlabel = 'PurchaseOrderCustomReferenceNumber';
	$uploaddir = $conf->purchaseordercustomreferencenumber->dir_output;
...
$title = $langs->trans("PurchaseOrderCustomReferenceNumbers");
//$help_url = "EN:Module_PurchaseOrderCustomReferenceNumber|FR:Module_PurchaseOrderCustomReferenceNumber_FR|ES:Módulo_PurchaseOrderCustomReferenceNumber";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/purchaseordercustomreferencenumber/purchaseordercustomreferencenumber_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-purchaseordercustomreferencenumber page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/purchaseordercustomreferencenumber/purchaseordercustomreferencenumber_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendPurchaseOrderCustomReferenceNumberRef";
$modelmail = "purchaseordercustomreferencenumber";
$objecttmp = new PurchaseOrderCustomReferenceNumber($db);
