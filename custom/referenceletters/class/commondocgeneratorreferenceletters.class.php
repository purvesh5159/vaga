...
if (!empty($arrayidcontact)) {
    foreach ($arrayidcontact as $idsale) {
		...
		// Fetch job position, phone, and email
        $jobPosition = $object->user->job; // Assumes 'job' stores the job position.
        $contactPhone = $object->user->office_phone; // Assumes 'office_phone' stores the phone number.
        $contactEmail = $object->user->email; // Assumes 'email' stores the email.
        
        // Append contact details (name, job position, phone, email)
        $resarray[$array_key . '_contactsale'] 
          .= ($resarray[$array_key . '_contactsale'] ? "<br />" : '') // Add a <br /> if this isn't the first contact
          . $outputlangs->convToOutputCharset($object->user->getFullName($outputlangs, 1)) // Line 1: Name
          . (!empty($jobPosition) ? "<br />" . $outputlangs->convToOutputCharset($jobPosition) : '') // Line 2: Job Position (if available)
          . (!empty($contactPhone) ? "<br />Phone: " . $outputlangs->convToOutputCharset($contactPhone) : '') // Line 3: Phone (if available)
          . (!empty($contactEmail) ? "<br />Email: " . $outputlangs->convToOutputCharset($contactEmail) : '') // Line 4: Email (if available)
          . "<br />"; // Newline after each contact
          
        // Add amount in currency
        $amount = $object->total_ttc;    // objvar_object_total_ttc 11000000.75; // Replace with the dynamic amount
        $amountInCurrency = $this->numberToCurrency($amount, 'Dollars');
        $resarray[$array_key . '_amount_in_currency'] = $outputlangs->convToOutputCharset($amountInCurrency);
        
        
        // Determine the shipping method ID
...
// Fetch the shipping method details
...
// Assign to a separate resarray key
$resarray['shipping_method_code'] = $shipping_method_code;
$resarray['shipping_method_label'] = $shipping_method_label;

// OPTIONAL: Use $resarray['shipping_method_label'] wherever necessary to display it in your document.

        
        
        //echo $resarray['line_up_locale'];
        //die();
            //$resarray['line_up_locale'] =  number_format($resarray['line_up_locale'], 2);
           //$resarray['line_price_ht_locale'] =  number_format($resarray['line_price_ht_locale'], 2);
    }
        ...
        foreach ( $object->lines as &$line ) {
           // echo $resarray['line_up_locale'];
         //   echo number_format($resarray['line_up_locale'], 2);
            //die();
    ...
    $resarray['objvar_object_total_ht'] = number_format($object->total_ht, 2, '.', '');
    $resarray['objvar_object_total_ttc'] = number_format($object->total_ttc, 2, '.', '');
	...
	public function get_substitutionarray_other($outputlangs, $object = '') {
		...
		$array_other['tva_detail_montants'] = number_format($object->total_tva, 2, '.', '');
 ...
 // Increment and use the class variable for serial number
    $this->slno++;
    $resarray['serial_number'] = $this->slno;
    ...
    // Add other line details
    $resarray['line_fulldesc'] = $line->desc;
   // $resarray['line_qty'] = number_format($line->qty, 2, '.', '');
    $resarray['line_up_locale'] = number_format($line->subprice, 2, '.', '');
    $resarray['line_price_ht_locale'] = number_format($line->total_ht, 2, '.', '');
...
//			$value=implode(' ', $toprint);
    ...
    public function numberToWords($num)
    {
        ...
        foreach ($chunks as $chunk) {
            ...
            if ($chunk) {
                $chunkWords = [];
                if ($chunk >= 100) {
                    $chunkWords[] = $words[intval($chunk / 100)] . ' Hundred';
                    $chunk %= 100;
                }
                if ($chunk >= 20) {
                    $chunkWords[] = $words[intval($chunk / 10) * 10];
                    $chunk %= 10;
                }
                if ($chunk) {
                    $chunkWords[] = $words[$chunk];
                }
                $wordsArray[] = implode(' ', $chunkWords) . ' ' . $scales[$scaleIndex];
            }
            $scaleIndex--;
        }
	...
	/**
 * Convert a number to currency format in words
 * @param float|int $num The number to convert
 * @param string $currency The name of the currency (default: Dollars)
 * @return string The number in currency words
 */
public function numberToCurrency($num, $currency = 'Dollars')
{
    $whole = floor($num); // Whole number part
    $fraction = round(($num - $whole) * 100); // Fractional part as cents

    // Convert the whole number part to words
    $wholeWords = $this->numberToWords($whole);

    // Handle the fractional part
    $fractionWords = '';
    if ($fraction > 0) {
        $fractionWords = ' and ' . $this->numberToWords($fraction) . ' Cents';
    } else {
        $fractionWords = ' and Zero Cents';
    }

    // Construct the currency string
    $currencyString = $wholeWords . ' ' . $currency . $fractionWords;
