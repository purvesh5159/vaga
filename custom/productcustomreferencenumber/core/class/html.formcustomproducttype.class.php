...
Class FormCustomProductType
{
    ...
    public function selectCustomProductType($selected = '', $htmlname = 'customproducttype', $filtertype = '', $format = 0, $empty = 0, $noadmininfo = 0, $maxlength = 0, $morecss = '', $multiselect = 0)
	{
		global $langs, $user;

		$selected = is_array($selected) ? $selected : (!empty($selected) ? explode(',', $selected) : array());
		$customproducttypestat = new ProductCustomReferenceNumber($this->db);
		...
		$customproducttypestat->loadCacheTypesCustomProduct();
		...
		if (is_array($customproducttypestat->cache_types_custom_product) && count($customproducttypestat->cache_types_custom_product)) {
			foreach ($customproducttypestat->cache_types_custom_product as $id => $arraytypes) {
