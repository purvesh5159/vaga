...
/**
 *   	\file       productcustomreferencenumber_list.php
 *		\ingroup    productcustomreferencenumber
 *		\brief      List page for productcustomreferencenumber
 */
...
require_once __DIR__.'/class/productcustomreferencenumber.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('productcustomreferencenumber', 'productcustomreferencenumber', 'write');
	$permissiontodelete = $user->hasRight('productcustomreferencenumber', 'productcustomreferencenumber', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("productcustomreferencenumber")) {
	accessforbidden('Module productcustomreferencenumber not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'ProductCustomReferenceNumber';
	$objectlabel = 'ProductCustomReferenceNumber';
	$uploaddir = $conf->productcustomreferencenumber->dir_output;
...
$title = $langs->trans("ProductCustomReferenceNumbers");
//$help_url = "EN:Module_ProductCustomReferenceNumber|FR:Module_ProductCustomReferenceNumber_FR|ES:Módulo_ProductCustomReferenceNumber";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/productcustomreferencenumber/productcustomreferencenumber_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-productcustomreferencenumber page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/productcustomreferencenumber/productcustomreferencenumber_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendProductCustomReferenceNumberRef";
$modelmail = "productcustomreferencenumber";
$objecttmp = new ProductCustomReferenceNumber($db);
