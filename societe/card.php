	...
	if ($action == 'create') {
		...
		if (!empty($conf->use_javascript_ajax)) {
			...
			print '$(document).ready(function () {
				    ...
				    function init_customer_categ() {
						const customerProspectVal = jQuery("#customerprospect").val();
						console.log("is customer or prospect = " + customerProspectVal);

						if (customerProspectVal == -1 || customerProspectVal == 0) {
					...
					function init_supplier_categ() {
						...
						if (jQuery("#fournisseur").val() == 0 || jQuery("#fournisseur").val() == -1) {
		...
		if (empty($reshook)) {
			...
			*/
			...
			print '<td class="visibleifcustomer">'.$form->editfieldkey('CustomerCode', 'customer_code', '', $object, 0).'</td><td class="visibleifcustomer">';
			...
			if ((isModEnabled("fournisseur") && $user->hasRight('fournisseur', 'lire') && !getDolGlobalString('MAIN_USE_NEW_SUPPLIERMOD')) || (isModEnabled("supplier_order") && $user->hasRight('supplier_order', 'lire')) || (isModEnabled("supplier_invoice") && $user->hasRight('supplier_invoice', 'lire'))
				...
				print '<td class="visibleifsupplier">';
				if ((isModEnabled("fournisseur") && $user->hasRight('fournisseur', 'lire') && !getDolGlobalString('MAIN_USE_NEW_SUPPLIERMOD')) || (isModEnabled("supplier_order") && $user->hasRight('supplier_order', 'lire')) || (isModEnabled("supplier_invoice") && $user->hasRight('supplier_invoice', 'lire'))) {
					print $form->editfieldkey('SupplierCode', 'supplier_code', '', $object, 0);
				}
				print '</td><td class="visibleifsupplier">';
				if ((isModEnabled("fournisseur") && $user->hasRight('fournisseur', 'lire') && !getDolGlobalString('MAIN_USE_NEW_SUPPLIERMOD')) || (isModEnabled("supplier_order") && $user->hasRight('supplier_order', 'lire')) || (isModEnabled("supplier_invoice") && $user->hasRight('supplier_invoice', 'lire'))) {
					print '<table class="nobordernopadding"><tr><td>';
...
$(document).ready(function () {
    function updateBarcode() {
        const isVendor = $("#fournisseur").is(":checked") || $("#fournisseur").val() == "1";
        const vendorCode = $("#supplier_code").val();
        const customerCode = $("#customer_code").val();

        if (isVendor && vendorCode) {
            $("#barcode").val(vendorCode);
        } else if (customerCode) {
            $("#barcode").val(customerCode);
        }
    }
    ...
    $("#fournisseur, #name, #supplier_code, #customer_code").on("change keyup", function () {
        updateBarcode();
    });
});
</script>';
		...
		if ($socid) {
			...
			if (empty($reshook)) {
				...
				print '<td class="visibleifcustomer">'.$form->editfieldkey('CustomerCode', 'customer_code', '', $object, 0).'</td><td>';

				print '<table class="nobordernopadding"><tr><td class="visibleifcustomer">';
