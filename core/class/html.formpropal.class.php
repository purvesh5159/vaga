	...
	public function selectProposalStatus($selected = '', $short = 0, $excludedraft = 0, $showempty = 1, $mode = 'customer', $htmlname = 'propal_statut', $morecss = '')
	{
		...
		if ($mode == 'supplier') {
			...
			$listofstatus = array(
				...
				5=>array('id'=>5, 'code'=>'PR_LOST'),
				6=>array('id'=>6, 'code'=>'PR_CANCELLED'),
				7=>array('id'=>7, 'code'=>'PR_REVISED')
			);
		} else {
			$prefix = "PropalStatus";
		...
		foreach ($listofstatus as $key => $obj) {
			...
			if ($langs->trans($prefix.$key.($short ? 'Short' : '')) != $prefix.$key.($short ? 'Short' : '')) {
				...
				$conv_to_new_code = array('PR_DRAFT'=>'Draft', 'PR_OPEN'=>'Validated', 'PR_CLOSED'=>'Closed', 'PR_SIGNED'=>'Signed', 'PR_NOTSIGNED'=>'NotSigned', 'PR_FAC'=>'Billed', 'PR_LOST'=>'Lost', 'PR_CANCELLED'=>'Cancelled', 'PR_REVISED'=>'Revised');
