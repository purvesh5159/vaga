...
<!-- Title with version -->
<div class="login_table_title center" 
title="<?php //echo dol_escape_htmltag($title); 
echo "VAGA";
?>">
<?php
if ($disablenofollow) {
	//echo '<a class="login_table_title" href="#" target="_blank" rel="noopener noreferrer external">';
	echo '<a class="login_table_title" href="https://www.vagainc.ca" target="_blank" rel="noopener noreferrer external">';
}
//echo dol_escape_htmltag($title);
echo "Welcome to Vaga Refrigeration Inc.";
if ($disablenofollow) {
	echo '</a>';
}
?>
...
// Enable Google Authenticator Code here
if (!empty($gacaptcha)) {
	// Add a variable param to force not using cache (jmobile)
	$php_self = preg_replace('/[&\?]time=(\d+)/', '', $php_self); // Remove param time
	if (preg_match('/\?/', $php_self)) {
	...
	<span class="fa fa-unlock"></span>
	<span class="span-icon-security inline-block">
	<input id="gasecuritycode" placeholder="<?php echo $langs->trans("GoogleAuthenticatorCode"); ?>" class="flat input-icon-security width170" type="text" maxlength="6" name="gacode" tabindex="3" autocomplete="off" />
	</span>
	<span class="nowrap inline-block">
	<img class="inline-block valignmiddle hidden" src="<?php echo DOL_URL_ROOT ?>/core/antispamimage.php" border="0" width="80" height="32" id="img_securitycode" />
	<a class="inline-block valignmiddle" href="<?php echo $php_self; ?>" tabindex="4" data-role="button"><?php echo $gacaptcha_refresh; ?></a>
	</span>
