...
if (!function_exists('dol_loginfunction')) {
	...
	function dol_loginfunction($langs, $conf, $mysoc)
	{
		...
		*/
		...
		// Security Google Authenticator code
		$gacaptcha = 0;
		$gacaptcha_refresh = '';
		if (function_exists("imagecreatefrompng") && !empty($conf->global->MAIN_SECURITY_ENABLECAPTCHA1)) {
			$gacaptcha = 1;
			$gacaptcha_refresh = img_picto($langs->trans("Refresh"), 'refresh', 'id="gacaptcha_refresh_img"');
		}	
