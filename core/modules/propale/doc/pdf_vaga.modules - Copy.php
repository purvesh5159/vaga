...
if (!empty($object->array_options['options_warranty'])) {
    ...
    $pdf->SetFont('', '');
    $pdf->MultiCell($this->page_largeur - 100, 4, $warranty, 0, 'L', 0, 1, $this->marge_gauche + 42, $posy, true);

    $posy = $pdf->GetY() + 3; // Update position dynamically
}

// 2. Scope of Work Not Included Section
if (!empty($object->array_options['options_scopesarenotincluded'])) {
    $scopesarenotincluded = $object->array_options['options_scopesarenotincluded'];

    $pdf->SetFont('', 'B');
    $pdf->MultiCell($this->page_largeur - 25, 4, "Scope of Work Not Included:", 0, 'L', 0, 1, $this->marge_gauche, $posy, true);

    $pdf->SetFont('', '');
    $pdf->MultiCell($this->page_largeur - 25, 4, $scopesarenotincluded, 0, 'L', 0, 1, $this->marge_gauche + 42, $posy, true);

    $posy = $pdf->GetY() + 3;
}

// Check if the current position exceeds the printable area
if ($posy > ($pdf->getPageHeight() - $this->marge_basse)) {
    $pdf->AddPage();
    $posy = $this->marge_haute; // Reset position for the new page
}
...
if (!empty($object->array_options['options_tentativeleadtime'])) {
    ...
    $pdf->SetFont('', 'B');
    $pdf->MultiCell($this->page_largeur - 25, 4, "Tentative Lead Time:", 0, 'L', 0, 1, $this->marge_gauche, $posy, true);
...
// 4. INCOTERMS BELOW INFO TABLE
if (isModEnabled('incoterm')) {
    $desc_incoterms = $object->getIncotermsForPDF(); // Retrieve incoterm details
    if ($desc_incoterms) {
        // Title
        $pdf->SetFont('', 'B');
        $pdf->MultiCell($this->page_largeur - 25, 4, "Incoterms: ", 0, 'L', 0, 1, $this->marge_gauche, $posy, true);

        // Description
        $pdf->SetFont('', '');
        $pdf->MultiCell(
            $this->page_largeur - 25, 
            4, 
            str_replace("Incoterm :", "", $desc_incoterms), 
            0, 'L', 0, 1, 
            $this->marge_gauche + 42, $posy, true
        );

        // Update position based on the content's dynamic height
        $posy = $pdf->GetY() + 3; // Add extra space after the content
    }
}
               ...
               /// }
				...
				//5. SHIPPING METHODS and 6. PAYMENT TERMS coming from drawinfo table
				
				// 5. DELIVERY MODE BELOW INFO TABLE
					if (!getDolGlobalString('PROPOSAL_PDF_HIDE_DELIVERYMODE') && $object->shipping_method_id > 0) {
						$outputlangs->load("sendings");

						// Determine the shipping method ID and adjust if global settings are enabled
						$shipping_method_id = $object->shipping_method_id;
						if (getDolGlobalString('SOCIETE_ASK_FOR_SHIPPING_METHOD') && !empty($this->emetteur->shipping_method_id)) {
							$shipping_method_id = $this->emetteur->shipping_method_id;
						}
						...
						$pdf->MultiCell(43, 4, $outputlangs->transnoentities("SendingMethod") . ':', 0, 'L', 0, 1, $this->marge_gauche, $posy, true);

						// Description
						$pdf->SetFont('helvetica', '', 8);
						$lib_condition_paiement = ($outputlangs->transnoentities("SendingMethod" . strtoupper($shipping_method_code)) != "SendingMethod" . strtoupper($shipping_method_code)) 
				...
				// 6. PAYMENT CONDITIONS BELOW INFO TABLE
				if (!getDolGlobalString('PROPOSAL_PDF_HIDE_PAYMENTTERM') && $object->cond_reglement_code) {
					// Load necessary font settings and positions
					$pdf->SetFont('helvetica', 'B', 8);
					$pdf->MultiCell(
						43, 
						4, 
						$outputlangs->transnoentities("PaymentConditions") . ':', 
						0, 'L', 0, 1, 
						$this->marge_gauche, $posy, true
					);
					...
					// Determine payment condition label
					$lib_condition_paiement = $outputlangs->transnoentities("PaymentCondition" . $object->cond_reglement_code) 
						!= 'PaymentCondition' . $object->cond_reglement_code 
						? $outputlangs->transnoentities("PaymentCondition" . $object->cond_reglement_code) 
						: $outputlangs->convToOutputCharset(
							$object->cond_reglement_doc ? $object->cond_reglement_doc : $object->cond_reglement_label
						);

					// Handle dynamic deposit percentage substitution
					if ($object->deposit_percent > 0) {
						$lib_condition_paiement = str_replace('__DEPOSIT_PERCENT__', $object->deposit_percent, $lib_condition_paiement);
					}

					// Replace any newline placeholders and output the text
					$lib_condition_paiement = str_replace('\n', "\n", $lib_condition_paiement);
					$pdf->MultiCell(
						67, 
						4, 
					...
					// Adjust position for subsequent content
					$posy = $pdf->GetY() + 3; // Add extra spacing
				}
