 ...
 */
// Adjust error reporting to suppress deprecated warnings and notices
...
if (!defined('NOLOGIN')) {
	...
	// Verification Google Authenticator graphic code
		if ($test && GETPOST("username", "alpha", 2) && getDolGlobalString('MAIN_SECURITY_ENABLECAPTCHA1') && !isset($_SESSION['dol_bypass_antispam'])) {
			$sessionkey = 'dol_antispam_value';
			...
			require($currentDirectory . '/google-authenticator/vendor/autoload.php');
			$secret_key = 'WU2N6ZAMYPSXPMWWBPQGFDHY43F7ZT55';
			$user_provided_code = GETPOST('gacode', 'restricthtml');
		...
		//	$ok = (array_key_exists($sessionkey, $_SESSION) === true && (strtolower($_SESSION[$sessionkey]) === strtolower(GETPOST('gacode', 'restricthtml'))));													
$ok = (array_key_exists($sessionkey, $_SESSION) === true && ($valid == "valid"));
			// Check code
			if (!$ok) {
				dol_syslog('Bad value for code, connexion refused. Google 2FA failed. ', LOG_NOTICE);
				// Load translation files required by page
				$langs->loadLangs(array('main', 'errors'));

				$_SESSION["dol_loginmesg"] = $langs->transnoentitiesnoconv("ErrorBadValueForCode" ."  -  " .$valid ."  -  " .$user_provided_code);
				$test = false;
