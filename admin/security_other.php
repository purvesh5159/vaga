...
// Enable Google Authenticator code
print '<tr class="oddeven">';
print '<td colspan="3">'.$langs->trans("UseGAuthenticatorCode").'</td>';
print '<td class="right">';
if (function_exists("imagecreatefrompng")) {
	if (!empty($conf->use_javascript_ajax)) {
		print ajax_constantonoff('MAIN_SECURITY_ENABLECAPTCHA1');
		
	} else {
		
		if (empty($conf->global->MAIN_SECURITY_ENABLECAPTCHA1)) {
			print '<a href="'.$_SERVER['PHP_SELF'].'?action=set_MAIN_SECURITY_ENABLECAPTCHA1&token='.newToken().'">'.img_picto($langs->trans("Disabled"), 'off').'</a>';
		} else {
			print '<a href="'.$_SERVER['PHP_SELF'].'?action=del_MAIN_SECURITY_ENABLECAPTCHA1&token='.newToken().'">'.img_picto($langs->trans("Enabled"), 'on').'</a>';
		}
	}
} else {
	$desc = $form->textwithpicto('', $langs->transnoentities("EnableGDLibraryDesc"), 1, 'warning');
