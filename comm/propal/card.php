...
if (empty($reshook)) {
		...
		if (!($socid > 0)) {
			...
			if ($object->id > 0) {
				...
				if ($result > 0) {
					 ...
					 // FETCH the new cloned proposal PURVESH proposal revision
						$newpropal = new Propal($db);
						$newpropal->fetch($result);

						// --- REVISION HANDLING ---
						// Extract base reference (without -R#)
						$base_ref = preg_replace('/-R\d+$/', '', $object->ref);

						// Get max revision number for base reference
						$sql = "SELECT MAX(revision) as max_rev FROM ".MAIN_DB_PREFIX."propal
								WHERE ref LIKE '".$db->escape($base_ref)."%' AND ref NOT LIKE '".$db->escape($base_ref)."-%-%'";
						$resql = $db->query($sql);
						$revision = 1;
						if ($resql && ($obj = $db->fetch_object($resql))) {
							$revision = ((int) $obj->max_rev) + 1;
						}

						// Generate new ref with incremented revision
						$new_ref = $base_ref . '-R' . $revision;

						// Update the new proposal with new ref and revision
						$newpropal->ref = $new_ref;
						$newpropal->revision = $revision;
						$object->statut = Propal::STATUS_VALIDATED;
				     	$object->status = Propal::STATUS_VALIDATED;

						// Save changes
						$newpropal->update($user);

						// Set other proposals with same base_ref to 'obsolete'
						$sql = "UPDATE ".MAIN_DB_PREFIX."propal
								SET fk_statut = '9'
								WHERE ref LIKE '".$db->escape($base_ref)."%' AND rowid != ".((int) $newpropal->id);
						$db->query($sql);
	...
	/*purvehs update status as cancelled */
	
	/*elseif ($action == 'setstatuscancelled' && $user->rights->propale->creer)
	{
		$object->statut = Propal::STATUS_CANCELLED;
		...
		elseif (GETPOST('action') == 'update_propal_link' && $user->rights->propale->creer) {
			$linked_propal_id = GETPOST('linked_propal_id', 'int');
			$link_reason = GETPOST('link_reason', 'alpha');
			
			if ($linked_propal_id) {
				// Check if link already exists
				$sql_check = "SELECT rowid FROM ".MAIN_DB_PREFIX."propal_links";
				$sql_check .= " WHERE fk_propal_source = ".(int)$object->id;
				$sql_check .= " AND fk_propal_linked = ".(int)$linked_propal_id;
				
				$resql_check = $db->query($sql_check);
				
				if ($db->num_rows($resql_check) == 0) {
					// Insert new link
					$sql_insert = "INSERT INTO ".MAIN_DB_PREFIX."propal_links";
					$sql_insert .= " (fk_propal_source, fk_propal_linked, reason, date_creation, fk_user_creat)";
					$sql_insert .= " VALUES (".(int)$object->id.", ".(int)$linked_propal_id.", '".$db->escape($link_reason)."', '".$db->idate(dol_now())."', ".(int)$user->id.")";
					$result = $db->query($sql_insert);

					if ($result) {
						setEventMessages('Proposal linked successfully', null, 'mesgs');
						header("Location: ".$_SERVER["PHP_SELF"]."?id=".$object->id);
					   exit;
					} else {
						setEventMessages('Error linking proposal: '.$db->lasterror(), null, 'errors');
					}
				} else {
					setEventMessages('This proposal is already linked', null, 'warnings');
				}
			
			} else {
				setEventMessages('Please select a proposal and provide a reason', null, 'errors');
			}
		...
		elseif (GETPOST('action') == 'unlink_propal' && $user->rights->propale->creer) {
			$link_id = GETPOST('link_id', 'int');
			
			if ($link_id) {
				$sql_delete = "DELETE FROM ".MAIN_DB_PREFIX."propal_links WHERE rowid = ".(int)$link_id;
				
				if ($db->query($sql_delete)) {
					setEventMessages('Proposal unlinked successfully', null, 'mesgs');
					header("Location: ".$_SERVER["PHP_SELF"]."?id=".$object->id);
					   exit;
				} else {
					setEventMessages('Error unlinking proposal', null, 'errors');
				}
	...
	elseif ($action == 'setstatus' && $user->rights->propale->creer)
	{
		$object->fetch($id);
	...
	/*elseif ($action == 'setstatusrevised' && $user->rights->propale->creer)
	{
		$object->fetch($id);
		$object->fetch_optionals();
	
		$object->array_options['options_proposalrevision'] = trim(GETPOST('options_proposalrevision', 'alphanohtml'));
	
		$object->statut = Propal::STATUS_REVISED;
		$object->update($user);
...
if ($action == 'create') {
	...
	if ($action != 'presend') {
		...
		if (empty($reshook)) {
			if ($action != 'editline') {
				...
				// REVISED purvesh
				// print '<div>';
				// print '<form method="POST" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" style="display:inline-block; margin-bottom: 10px;">';
				// print '<input type="hidden" name="token" value="'.newToken().'">';
				// print '<input type="hidden" name="action" value="setstatusrevised">';

				// print '<select name="options_proposalrevision" class="flat minwidth100 maxwidthonsmartphone select2" style="margin-right: 5px;">';
				// for ($i = 0; $i <= 10; $i++) {
				// 	print '<option value="R'.$i.'">R'.$i.'</option>';
				// }
				// print '</select>';

				// print '<input class="button butAction small" type="submit" value="Mark as Revised">';
				// print '</form>';
				// print '</div>';


				//cancelled purvesh
				/* print '<form method="POST" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" style="display:inline-block; margin-right: 10px;">';
                 print '<input type="hidden" name="token" value="'.newToken().'">';
				 print '<input type="hidden" name="action" value="setstatuscancelled">';
				 print '<input class="button small" type="submit" value="Mark as Cancelled">';
				 print '</form>';*/
		...
		$sql = "SELECT p.rowid, p.ref, p.ref_client, p.datep, p.fin_validite";
		$sql .= " FROM ".MAIN_DB_PREFIX."propal p";
		$sql .= " WHERE p.fk_soc = ".(int) $soc->id;
		$sql .= " AND p.entity IN (".getEntity('propal').")";
	...
	// Status change form with reason dropdown pruvesh
		...
		print '<th colspan="5">Update Proposal Status</th>';
		print '</tr>';
	...
	//	print '<td class="titlefield">Link to Proposal</td>';
		...
		print '<option value="2"'.($object->statut == 2 ? ' selected' : '').'>Signed</option>';
		print '<option value="3"'.($object->statut == 3 ? ' selected' : '').'>Not Signed</option>';
		print '<option value="4"'.($object->statut == 4 ? ' selected' : '').'>Billed</option>';
		print '<option value="5"'.($object->statut == 5 ? ' selected' : '').'>Lost</option>';
		print '<option value="6"'.($object->statut == 6 ? ' selected' : '').'>Cancelled</option>';
		print '<option value="7"'.($object->statut == 7 ? ' selected' : '').'>Revised</option>';
		print '<option value="8"'.($object->statut == 8 ? ' selected' : '').'>Closed</option>';
		print '<option value="9"'.($object->statut == 9 ? ' selected' : '').'>In Active</option>';
		print '<option value="10"'.($object->statut == 10 ? ' selected' : '').'>Review</option>';
		print '<option value="11"'.($object->statut == 11 ? ' selected' : '').'>Pending</option>';
		print '</select>';
		print '</div>';
		print '<div class="inline-block" style="vertical-align: top; margin-right: 10px;">';
		...
		print '<table class="noborder centpercent">';
		print '<tr class="liste_titre">';
		print '<th colspan="5">Linked Proposals</th>';
		print '</tr>';
	...
	//	print '<td class="titlefield">Link to Proposal</td>';
		...
		print '<div class="inline-block" style="vertical-align: top; margin-right: 10px;">';
		print '<select name="linked_propal_id" class="flat minwidth300 maxwidthonsmartphone">';
		...
		foreach ($propal_refs as $propal) {
			...
			if ($object->fk_propal_linked == $propal['rowid']) $selected = ' selected';
			
			$option_text = $propal['ref'];
			if ($propal['ref_client']) $option_text .= ' ('.$propal['ref_client'].')';
			if ($propal['title']) $option_text .= ' - '.dol_trunc($propal['title'], 40);
			$option_text .= ' - '.dol_print_date($db->jdate($propal['datep']), 'day');
		...
		print '</div>';
		print '<div class="inline-block" style="vertical-align: top;">';
		print '<input class="button small" type="submit" value="Link Proposal">';
		print '</div>';
		print '</form>';
		print '</td>';
		print '</tr>';
		...
		$sql_linked = "SELECT pl.*, p.ref, p.ref_client, p.datep";
		$sql_linked .= " FROM ".MAIN_DB_PREFIX."propal_links pl";
		$sql_linked .= " LEFT JOIN ".MAIN_DB_PREFIX."propal p ON p.rowid = pl.fk_propal_linked";
		$sql_linked .= " WHERE pl.fk_propal_source = ".(int)$object->id;
		...
		if ($resql_linked) {
			...
			if ($num_linked > 0) {
				while ($obj_linked = $db->fetch_object($resql_linked)) {
						...
						print '<td class="left"><a href="card.php?id='.$obj_linked->fk_propal_linked.'">'.$obj_linked->ref.'</a></td>';
						print '<td class="left">'.($obj_linked->ref_client ? $obj_linked->ref_client : '').'</td>';
						print '<td class="left">'.dol_print_date($db->jdate($obj_linked->datep), 'day').'</td>';
