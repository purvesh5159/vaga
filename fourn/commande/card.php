...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $usercancreate) {
		if (1 == 0 && !GETPOST('clone_content') && !GETPOST('clone_receivers')) {
			...
			if ($object->id > 0) {
				...
				$neworder = new CommandeFournisseur($db);
				$neworder->fetch($result);
                $original = $orig->ref;
				$new_ref = substr($original, 0, strrpos($original, '-'));
			
				$base_ref = preg_replace('/-R\d+$/', '', $new_ref);
            
				$sql = "SELECT MAX(revision) as maxrev FROM ".MAIN_DB_PREFIX."commande_fournisseur
				...
				$object->statut = CommandeFournisseur::STATUS_VALIDATED;
				$object->status = CommandeFournisseur::STATUS_VALIDATED;
				$neworder->update($user);

				// Mark older revisions obsolete
				$sql2 = "UPDATE ".MAIN_DB_PREFIX."commande_fournisseur
						SET fk_statut = 10
						WHERE ref LIKE '".$db->escape($base_ref)."%' 
	...
	// Handle form submissions purvesh
		elseif (GETPOST('action') == 'update_propal_link' && $user->rights->fournisseur->commande->creer) {
			$linked_comm_fourni_id = GETPOST('linked_comm_fourni_id', 'int');
			$link_reason = GETPOST('link_reason', 'alpha');
			
			if ($linked_comm_fourni_id) {
				// Check if link already exists
				$sql_check = "SELECT rowid FROM ".MAIN_DB_PREFIX."commande_fournisseur_links";
				$sql_check .= " WHERE fk_comm_fourni_source  = ".(int)$object->id;
				$sql_check .= " AND fk_comm_fourni_linked  = ".(int)$linked_comm_fourni_id;
				
				$resql_check = $db->query($sql_check);
				
				if ($db->num_rows($resql_check) == 0) {
					// Insert new link
					$sql_insert = "INSERT INTO ".MAIN_DB_PREFIX."commande_fournisseur_links";
					$sql_insert .= " (fk_comm_fourni_source, fk_comm_fourni_linked , reason, date_creation, fk_user_creat)";
					$sql_insert .= " VALUES (".(int)$object->id.", ".(int)$linked_comm_fourni_id.", '".$db->escape($link_reason)."', '".$db->idate(dol_now())."', ".(int)$user->id.")";
					$result = $db->query($sql_insert);

					if ($result) {
						setEventMessages('Purchase Order linked successfully', null, 'mesgs');
						header("Location: ".$_SERVER["PHP_SELF"]."?id=".$object->id);
					   exit;
					} else {
						setEventMessages('Error linking Purchase Order: '.$db->lasterror(), null, 'errors');
					}
				} else {
					setEventMessages('This Purchase Order is already linked', null, 'warnings');
				}
			
			} else {
				setEventMessages('Please select a Purchase Order and provide a reason', null, 'errors');
			}
		...
		elseif (GETPOST('action') == 'unlink_propal' && $user->rights->fournisseur->commande->creer) {
			$link_id = GETPOST('link_id', 'int');
			
			if ($link_id) {
				$sql_delete = "DELETE FROM ".MAIN_DB_PREFIX."commande_fournisseur_links WHERE rowid = ".(int)$link_id;
				
				if ($db->query($sql_delete)) {
					setEventMessages('Purchase Order unlinked successfully', null, 'mesgs');
					header("Location: ".$_SERVER["PHP_SELF"]."?id=".$object->id);
					   exit;
				} else {
					setEventMessages('Error unlinking Purchase Order', null, 'errors');
				}
	...
	elseif ($action == 'setstatus' && $user->rights->fournisseur->commande->creer)
	{
		$object->fetch($id);
...
if ($action == 'create') {
	...
	if ($user->socid == 0 && $action != 'delete') {
		...
		$sql = "SELECT p.rowid, p.ref, p.ref_supplier, p.tms";
		$sql .= " FROM ".MAIN_DB_PREFIX."commande_fournisseur p";
	...
	// Status change form with reason dropdown pruvesh
		...
		print '<th colspan="5">Update Purchase Order Status</th>';
		print '</tr>';
	...
	//	print '<td class="titlefield">Link to Proposal</td>';
		...
		print '<option value="3"'.($object->statut == 3 ? ' selected' : '').'>Order Sent</option>';
		print '<option value="4"'.($object->statut == 4 ? ' selected' : '').'>Received Partially</option>';
		print '<option value="5"'.($object->statut == 5 ? ' selected' : '').'>Recevied Completely</option>';
		print '<option value="6"'.($object->statut == 6 ? ' selected' : '').'>Cancelled</option>';
		print '<option value="7"'.($object->statut == 7 ? ' selected' : '').'>Cancelled after order</option>';
		print '<option value="8"'.($object->statut == 9 ? ' selected' : '').'>Refused</option>';
		print '<option value="9"'.($object->statut == 10 ? ' selected' : '').'>In Active</option>';
		print '</select>';
		print '</div>';
		print '<div class="inline-block" style="vertical-align: top; margin-right: 10px;">';
		...
		print '<table class="noborder centpercent">';
		print '<tr class="liste_titre">';
		print '<th colspan="5">Linked Purchase Orders</th>';
		print '</tr>';
	...
	//	print '<td class="titlefield">Link to Proposal</td>';
		...
		foreach ($propal_refs as $propal) {
			...
			if ($object->fk_comm_fourni_linked == $propal['rowid']) $selected = ' selected';
			
			$option_text = $propal['ref'];
			if ($propal['ref_supplier']) $option_text .= ' ('.$propal['ref_supplier'].')';
		...
		print '</div>';
		print '<div class="inline-block" style="vertical-align: top;">';
		print '<input class="button small" type="submit" value="Link Purchase Order">';
		print '</div>';
		print '</div>';
		print '</form>';
		print '</td>';
		...
		print '<th class="left">Supplier Ref</th>';
		...
		$sql_linked = "SELECT pl.*, p.ref, p.ref_supplier, p.tms";
		$sql_linked .= " FROM ".MAIN_DB_PREFIX."commande_fournisseur_links pl";
		$sql_linked .= " LEFT JOIN ".MAIN_DB_PREFIX."commande_fournisseur p ON p.rowid = pl.fk_comm_fourni_linked ";
		$sql_linked .= " WHERE pl.fk_comm_fourni_source  = ".(int)$object->id;
		...
		if ($resql_linked) {
			...
			if ($num_linked > 0) {
				while ($obj_linked = $db->fetch_object($resql_linked)) {
						...
						print '<td class="left"><a href="card.php?id='.$obj_linked->fk_comm_fourni_linked.'">'.$obj_linked->ref.'</a></td>';
